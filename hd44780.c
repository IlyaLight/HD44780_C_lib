...
static void lcd_4takt_delay(volatile uint32_t us);
...
static volatile void lcd_4takt_delay(volatile uint32_t us)
{
	/* Conversion to us */
	for ( ; us > 0u; us--)//пустой цикл, 19 команда, (1.2us 16MHz)
	{
	}
}

/*!	\brief	Initiate the transfer of data/commands to LCD.  min - 230 nC*/
static void lcd_strobe(void)
{/* Low level function. */
	SET(LCD_E_OUT, LCD_E);
